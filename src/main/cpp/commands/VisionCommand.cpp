...
frc2::CommandPtr ForceAlignWithTarget(SubVision::Side side) {
  ...
  return SubDrivebase::GetInstance().Drive(
      [side] {
        if (SubVision::GetInstance().GetLastReefTagArea() >
            0) {  // 0 is for removing the move forward
          ...
          units::meters_per_second_t overallVelocity =
              std::clamp(0.16_mps * error.value(), -0.3_mps, 0.3_mps);
          ...
          frc::Rotation2d targetRotation =
              SubVision::GetInstance().GetReefPose(side, -1).Rotation();
...
frc2::CommandPtr AutoShootIfAligned(SubVision::Side side) {
  return Sequence(
      WaitUntil([side] {
        ...
        // vision tag angles
        if (tagAngle < goalAngle + visionTolerance * 1_deg &&
            tagAngle > goalAngle - visionTolerance * 1_deg) {
...
frc2::CommandPtr AutoShootIfKindaAligned(SubVision::Side side) {
  return Sequence(
      WaitUntil([side] {
        ...
        double drivebaseTolerance =
            Logger::Tune("AutoShoot/AutoShootDrivebaseLooseTolerance", 0.01);
        Logger::Log("AutoShoot/errorangle", (goalAngle - tagAngle).value());
